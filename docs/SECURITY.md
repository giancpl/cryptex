...
J2 serializes project writes across the process before fingerprint validation and atomic replacement, so concurrent callers cannot both commit from the same old fingerprint. Adversarial tests cover traversal, outside-root symlinks, post-read symlink substitution, failed and stale writes, outside-root watcher events, watcher overflow recovery, corrupt snapshots, restart recovery, and restrictive recovery permissions. See [adversarial evidence](ADVERSARIAL_TESTS.md).
